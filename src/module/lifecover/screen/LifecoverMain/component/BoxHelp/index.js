...
const BoxHelp = ({
  ...
  const dataGuide = useMemo(
    () => [
      ...
      },
      {
        title: 'ringkasanInfo',
        label: {
          id: <>Product & Service Information Summary (RIPLAY)</>,
          en: <>Ringkasan Informasi Produk & Layanan (RIPLAY)</>,
        },
  ...
  const handleRedirectHelper = useCallback((id) => {
    if (id === 'email') {
          ...
          <div className="flex flex-col gap-3">
            {dataHelp.map((item) => (
              <div
                key={item.id}
                role="button"
                onClick={() => handleRedirectHelper(item.id)}
