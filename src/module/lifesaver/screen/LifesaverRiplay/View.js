...
import { API, BASE_URL_PROD } from '@cp-util/constant';
import {
  LsRiplay1,
  LsRiplay2,
  LsRiplay3,
...
export default function Page(props) {
  const { showModalLsRiplay, setShowModalLsRiplay } = props;

  const router = useRouter();

  return (
    <Modal isOpen={showModalLsRiplay} size="full">
      <Container
        noBackground
        fullScreen
        ...
        <div className="fixed top-0 left-0 w-full bg-white border-b py-4 font-bold flex justify-center mb-4">
          <div className="flex w-full px-[5%] justify-between items-center">
            <div
              role="button"
              onClick={() => setShowModalLsRiplay(false)}
              className="">
