...
export default function Page(props) {
    ...
    (act) => {
      ...
      if (act === SET_SAVE_ADDRESS_FAILED) {
        ...
        if (setSaveAddressFailed?.message !== 'INTERNAL_SERVER_ERROR') {
          toast.alert('Warning', setSaveAddressFailed?.message);
        }
        setSaveAddressClear();
      }
      ...
      if (act === SET_UPDATE_ADDRESS_FAILED) {
        ...
        if (setUpdateAddressFailed?.message !== 'INTERNAL_SERVER_ERROR') {
          toast.alert('Warning', setUpdateAddressFailed?.message);
        }
        setUpdateAddressClear();
      }
    },
  ...
  const validateAddressTitle = useCallback(
    (text) => {
      ...
      if (text.length > 30) {
        setAddressTitleMessage({
          error: trans(locale, lang, 'addressTitleMaxLength'),
        });
        return false;
      }
  ...
  const renderFormAddAddress = () => {
    return (
      <div className="pt-5">
        <div className="w-full mb-4 lg:mb-6 flex flex-col md:flex-row justify-between">
        ...
        <div className="w-full mb-4 lg:mb-6 flex flex-col md:flex-row justify-between">
          <div className="w-full flex pt-1 justify-between md:w-[49%] mb-4 md:mb-0">
            <div className="w-[48%]">
              <Input
                value={rt}
                inputMode="numeric"
