...
const withOptimizedImages = (nextConfig = {}) => {
  return { ...nextConfig,
    ...
    webpack(config, options) {
      ...
      const handleImages = getHandledImageTypes(nextConfig.images && nextConfig.images.handleImages || ['jpeg', 'png', 'svg', 'webp', 'gif', 'ico']); // add optimized-images-loader
        ...
        config.module.rules.forEach(rule => {
          if (rule.oneOf) {
            rule.oneOf.forEach(subRule => {
              if (subRule.issuer && !subRule.test && !subRule.include && subRule.exclude && subRule.use && subRule.use.options && subRule.use.options.name) {
